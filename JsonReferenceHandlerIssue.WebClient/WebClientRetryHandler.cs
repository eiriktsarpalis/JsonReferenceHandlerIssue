...
{
    public class WebClientRetryHandler : DelegatingHandler
    {
        #region Fields
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WebClientRetryHandler"/> class to handle throttled requests (HTTP 429 Too many requests).
        ...
        public WebClientRetryHandler(int maxRetries = 3) : this(new HttpClientHandler(), maxRetries)
        {
        }
        ...
        /// <param name="maxRetries">Maximum number of retries before a request is failing.</param>
        public WebClientRetryHandler(HttpMessageHandler innerHandler, int maxRetries = 3)
            : base(innerHandler)
        {
            if (maxRetries < 0)
